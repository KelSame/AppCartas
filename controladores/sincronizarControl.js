...
exports.actualizarUsuario = async (req, res) => {
  try {
    const { usuarioId, usuario, correo, contrasena } = req.body;

    // Verificar si el nombre de usuario ya existe para otro usuario
    const usuarioExistente = await Usuario.findOne({ usuario, usuarioId: { $ne: usuarioId } });
    if (usuarioExistente) {
      return res.status(400).json({ error: 'El nombre de usuario ya está en uso por otro usuario.' });
    }

    // Verificar si el correo ya existe para otro usuario
    const correoExistente = await Usuario.findOne({ correo, usuarioId: { $ne: usuarioId } });
    if (correoExistente) {
    ...
    const actualizado = await Usuario.findOneAndUpdate(
      { usuarioId },
      { usuario, correo, ...(contrasena ? { contrasena } : {}) },
      { new: true }
    );
